...
* Raw data files in `DataRaw/Electricity` (multiple gigabytes in total)

As a result, the following script cannot be run without downloading from ENTSO-E (for generation) directly first (or by sending me a mail, we can try to find a solution):
